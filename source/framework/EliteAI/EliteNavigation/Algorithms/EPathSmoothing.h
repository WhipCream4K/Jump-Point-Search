	...
	public:
		...
		static std::vector<Elite::Vector2> OptimizePortals(const std::vector<Portal>& portals)
		{
			...
			for (unsigned int i = 1; i < static_cast<unsigned int>(portals.size()); ++i)
			{
				...
				auto crossProduct{ Elite::Cross(newRightLeg,rightLeg) };

				// TODO: I'm not sure what happen when newRightLeg align with oldRightLeg
				// this means the 'newRightLeg' is crossing the old one hence tunneling our funnel
				if(crossProduct <= 0.0f) 
				{
					//2. See if new line degenerates a line segment - RIGHT
					crossProduct = Elite::Cross(newRightLeg, leftLeg);
					if(crossProduct < 0.0f)
					{
						const Elite::Vector2 path{ apex + leftLeg };
						apex = path; // left leg becomes the apex
						...
						i = unsigned int(apexIndex);
						vPath.emplace_back(path);

						if(newIt < unsigned int(portals.size()))
						{
				...
				crossProduct = Elite::Cross(newLeftLeg, leftLeg);
				//--- LEFT CHECK ---
				//1. See if moving funnel inwards - LEFT
				if (crossProduct >= 0.0f)
				{
					//2. See if new line degenerates a line segment - LEFT
					crossProduct = Elite::Cross(newLeftLeg, rightLeg);
					if(crossProduct > 0.0f)
					{
						const Elite::Vector2 path{ apex + rightLeg };
						apex = path;
