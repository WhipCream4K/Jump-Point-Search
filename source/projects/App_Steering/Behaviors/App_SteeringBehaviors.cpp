...
{
	const auto firstAgent = AddAgent(BehaviorTypes::Seek, -1);
	const auto wanderAgent = AddAgent(BehaviorTypes::Wander, -1);
	const auto fleeAgent = AddAgent(BehaviorTypes::Evade, -1);
	wanderAgent.pAgent->SetPosition(Elite::Vector2(-5.0f,0.0f));
	fleeAgent.pAgent->SetPosition(Elite::Vector2(5.0f, 0.0f));
...
#pragma endregion
	...
	for (auto a : m_AgentVec)
	{
		if (a.pAgent)
		{
			a.pBehavior->PrintDebugOptions(agentTag);
